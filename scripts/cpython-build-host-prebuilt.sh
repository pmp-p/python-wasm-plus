...
if $HPY -c "import pymunk4"
then
    echo "
        * token-utils, pycparser, cffi, pymunk already installed
    " 1>&2
else
    ...
    then
        ...
        [ -d $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk4 ] && rm -rf $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk4

        rm -f  pymunk/libchipmunk*.so build/lib/pymunk/* chipmunk_src/*.so chipmunk_src/*/*.o

        $HPY setup.py build_chipmunk

        $SPY

        mv $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk/libchipmunk.so \
            $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk/libchipmunk64.so
        mv $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk \
            $HOST_PREFIX/lib/python${PYBUILD}/site-packages/pymunk4
        popd
    fi
